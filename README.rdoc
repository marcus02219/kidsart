...
==== Sign In(from social)
    ...
    results:
      {id,user_name,email,token, auth_id,social, avatar, state, phone}
    ex: curl -X POST -H "Content-Type: application/json" http://localhost:3000/api/v1/accounts/social_sign_in -d '{"email":"facebook@email.com","token":"asdfasdfasdfasdf", "social":"facebook"}'
==== Sign Out
